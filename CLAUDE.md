...
3. Os parâmetros de query/corpo chegam ao singleton `Request` **sem nenhuma sanitização** — `$_GET` e o corpo JSON decodificado são atribuídos diretamente. `Request::sanitizeParams()` (blocklist de regex em `Server\Interfaces\SQLInjectionPatterns`) ainda existe na classe, mas está marcado `@deprecated` e sua chamada foi removida do fluxo real (linha comentada no construtor de `Request`), pois corrompia silenciosamente valores legítimos (ver item 6 de `DIVIDA_TECNICA.md`). Não reintroduza essa chamada; a defesa real contra SQL injection deve vir de queries parametrizadas/prepared statements no código do consumidor do framework.
